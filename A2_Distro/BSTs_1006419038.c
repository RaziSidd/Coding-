...
BST_Node *newBST_Node(double freq, int bar, double index)
{
     ...
     ****/
	if (index < 0.0 || index > 1.0 || freq < 0.0 || bar < 0){
		return NULL;
	}
...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
{
	...
	double key = (10.0*(double)(new_node -> bar)+(double)(new_node -> index));
	
	if (root==NULL){
		return new_node;
	}// Tree is empty, new node becomes
	if (fabs(root -> key - (double)(key)) < 1e-15){
		printf("Duplicate node requested (bar:index)=%d,%lf, it was ignored\n",root -> bar, root -> index);
		return root;
	}
	if (root -> key > (double)(key)){
...
void BST_makePlayList(BST_Node *root)
{
	...
	//note *new_note1 = new_note(root -> freq,root ->bar,root ->index);
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
     ...
     ****/
	int found_first = 1000, found_second = 1000;
	for (int x = 0; x < 100; x++){
		if (strcmp(note_src, note_names[x]) == 0){
			found_first = x;
		}
		if (strcmp(note_dst, note_names[x]) == 0){
			found_second = x;
		}
	}
	if (found_first != 1000 || found_second != 1000){
		...
		if (note_freq[found_first] == root -> freq){
			root -> freq = note_freq[found_second];
		}   
...
Linked_List_Node *new_Linked_List_Node(double freq, int bar, double index)
{
	...
	Linked_List_Node* temp = (Linked_List_Node*)calloc(1,sizeof(Linked_List_Node));
    temp -> freq = freq;
	temp -> bar = bar;
	temp -> index = index;
	temp -> key = (10.0*bar)+index;
...
Linked_List_Node *Linked_List_insert(Linked_List_Node *head,double freq, int bar, double index)
{
    if (new_Linked_List_Node(freq,bar,index) != NULL){
			Linked_List_Node *new_node = new_Linked_List_Node(freq,bar,index);
			new_node -> next = head;
			return new_node;
	}
	return NULL;
...
Linked_List_Node *traverse_List(Linked_List_Node *head, BST_Node *root, int semitones, double time_shift)
{
    ...
    head = traverse_List(head, root->left, semitones, time_shift); 
	
	double new_freq, new_index;
	int found_first = 99;
	for (int x = 0; x < 100; x++){
		if (fabs(root -> freq - note_freq[x]) < 1e-15){
			found_first = x;
		}
	}
	if (!(found_first+semitones > 99 || found_first+semitones < 0)){
		new_freq = note_freq[found_first+semitones];
	}else{
		return head;
	}
	if (!(root -> index + time_shift < 0 || root -> index + time_shift >= 1)){
		...
		//printf("HERE\n");
		head = Linked_List_insert(head,new_freq,root->bar, new_index);
	}
    head = traverse_List(head, root->right,  semitones, time_shift); 
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     *    immediately next one in the musical scale - or what is the
	...
	head = traverse_List(head, root, semitones, time_shift);
	
	//printf("HERE3\n");
	Linked_List_Node *pointer = NULL;
	// if (head == NULL){
		// printf("its null\n");
	// }
	pointer = head;
	...
	while (pointer != NULL){
		...
		// printf("%d\n",pointer->bar);
		// printf("%f\n",pointer->key);
		BST_Node *temp = newBST_Node(pointer -> freq, pointer -> bar, pointer -> index);
		if (BST_search(root, temp -> bar, temp -> index) == NULL){
