 ...
 * 
 * We will be solving Sudoku sudokus. In case you have never
 ...
 * make up the large 9x9 sudoku. Another way to think
 ...
 * * RECURSIVELY!
 * 
 * (hint) - Solving a Sudoku with 9 unknown values
...
int solved(int sudoku[9][9])
{
   ...
   * are used once per row, column, and sub-sudoku.
...
int in_row(int sudoku[9][9], int row, int num){
...
int in_col(int sudoku[9][9], int col, int num) {
...
int in_three_by_three(int sudoku[9][9], int row, int col, int num) {
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
...
int put_here_alright(int sudoku[9][9], int row, int col, int num) {
	if (!in_row(sudoku, row, num) && !in_col(sudoku, col, num)&& !in_three_by_three(sudoku, row - (row % 3), col - (col %3), num)){
			return 1;
	}
	return 0;
}

int look_for_empty_one(int sudoku[9][9], int *row, int *col) {
	for (*row = 0; *row < 9; (*row)++) {
		for (*col = 0; *col < 9; (*col)++) {
			if (sudoku[*row][*col] == 0) {
				return 1;
...
int solve(int sudoku[9][9]) {
	...
	if (!look_for_empty_one(sudoku, &row, &col)){
		return 1;
	}
	
	for (int num = 1; num <= 9; num++ ) {
		
		if (put_here_alright(sudoku, row, col, num)) {
			sudoku[row][col] = num;
...
int main()
{
   ...
   // solution on different Sudoku sudokus.
