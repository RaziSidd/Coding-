...
int ingredient_index(char source_ingredient[MAX_STR_LEN])
{
	...
	for (int x = 0; x < MAT_SIZE; x++){
		if (strcmp(source_ingredient, ingredients[x]) == 0){
			return x;
		}
	}
    return -1;
...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
     ...
     * Implement this function
     * ********/
	int location = ingredient_index(source_ingredient);
	for (int j = 0; j < MAT_SIZE; j++ ){
		if (AdjMat[location][j] > 0){
			printf("%s \n", ingredients[j]);
		}
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
	...
	// Base case if k is zero, then we have to check around the 
	// given ingredent for any one that matches
	// if there is a match, check if it is in the list
	// if not add it in, and return
	
	// if k >= 1 
	// then we recurse to the ingredent to the side of it
	// making k - 1
	// once it returns, we add it to the list

	// try using the dist
	// first, get the nieghbors 
	// then send out to the related ones
	//  
	// printf(" dist %d \n", dist);
	// if (dist == 0){
		// printf("ITS 0000000000000000000000000000000000000000");
	// }
	
	
	if (dist == 0){
		//printf("ONLY CALLED ONCE\n");
	...
	if (k >= 1){
		int location = ingredient_index(source_ingredient);
		for (int j = 0; j < MAT_SIZE; j++ ){
			//printf("slower here 2\n");
			if (AdjMat[location][j] > 0){
				
				h = related_k_dist(h, source_ingredient, k-1, 1);
				h = related_k_dist(h, ingredients[j], k-1, 1);
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
     ...
     * 
     * intNode *head=NULL;
     * head=related_with_avoidance("rice", "nuts", 2, 1);
     * 
     * Should return a list of ingredients related to 'rice' by a distance
	...
	intNode *head = NULL, *head2 = NULL, *temp = NULL, *temp2 = NULL, *new_head = NULL;
	
	head = related_k_dist(head, source_ingredient, k_source, 0);
	
	int location = ingredient_index(avoid);
    head2 = insertInt(head2, location);
	...
	for(temp = head; temp != NULL ; temp = temp -> next){
		...
		if(is_in == 0){
			if (searchInt(new_head, temp -> x) == 0){
				new_head = insertInt(new_head, temp -> x);
			}
		}
...
int in_array(char recipe[10][MAX_STR_LEN], char checker[MAX_STR_LEN]){
	
	for (int x = 0;x < 10; x++){
		if (strcmp(recipe[x], checker)==0){
			return 1;
		}
	}
	return 0;
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
    ...
    ******/
	...
	// First, look thru the list to see how many of the ingredients are there
	// Second, create an array of the highest wieghted one
	// Then, add that one into the list where our old one was 
	
	// access the wieghts like in the above function 
	
	// we need to save the locations of the matrix, so we can re-add it in
	// we also need to make sure that the ingredent is not the same as any in the 	
	// recipe
	
	
	int num = 0,  avoid = 0, use_greatest = 0, use_j = 0;
	int greatest[10],  j_nums[10];
	//int locations[10];
	for (int x = 0;x < 10; x++){
		greatest[x] = 0;
	}
	...
	for (int x = 0; x < num; x++){
		//printf("Here2 %d \n", x);
		if (x != avoid){
			int location = ingredient_index(recipe[x]);
			for (int j = 0; j < MAT_SIZE; j++ ){
				if (AdjMat[location][j] > greatest[x]){
					if (strcmp(recipe[avoid], ingredients[j])!=0 && in_array(recipe, ingredients[j]) == 0){
						greatest[x] = AdjMat[location][j];
						//locations[x] = location;
						j_nums[x] = j;
						//nodes[x] = insertInt(nodes[x], j);
					}
				}	
			}
		}
	...
	// for (int x = 0; x < num-1; x++){
		// printf("=====this is one new List ============================\n");
		// printf("greatest %d \n", greatest[x]);
		// printf("ingredent %s \n", ingredients[j_nums[x]]);
		// printf("js %d \n", j_nums[x]);
		// printf("locations %d \n", locations[x]);

		// print_ingredients(nodes[x]);
		// printf("=====this is end new List ============================\n");

	// }
	
	for (int x = 0; x < num; x++){
		if (greatest[x] > use_greatest){
			use_greatest = greatest[x];
			use_j = j_nums[x];
		}
	}

	strcpy(recipe[avoid],ingredients[use_j]);
}
