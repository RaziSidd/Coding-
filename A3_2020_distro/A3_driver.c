...
gruyere cheese
   ...
   */
   ...
   // Time to practice those testing skills!
	...
	/* printf("\n ****** Related to 'medium shrimp' k-dist, k=0\n");
	head=related_k_dist(head,"medium shrimp",0,0);
	print_ingredients(head);
	head=deleteList(head);
   
    printf("\n ****** Related to 'tomato juice' k-dist, k=2\n");
	head=related_k_dist(head,"tomato juice",3,0);
	print_ingredients(head);
	head=deleteList(head);
 
	printf("\n ****** Related to 'medium shrimp' k-dist, k=1\n");
	head=related_k_dist(head,"medium shrimp",1,0);
	print_ingredients(head);
	head=deleteList(head);
	...
	printf("\n ****** Related with restrictions, 'soymilk', 'tomato juice', k_source=2, k_dest=0;\n");
	head=related_with_restrictions("soymilk","tomato juice",2,0);
	print_ingredients(head);
	head=deleteList(head);

	printf("\n ****** Related with restrictions, 'gruyere cheese', 'tomato juice', k_source=2, k_dest=2;\n");
	head=related_with_restrictions("gruyere cheese","tomato juice",2,2);
	print_ingredients(head);
	head=deleteList(head);

	printf("\n ****** substituting ingredient 'medium shrimp' in recipe\n");
	substitute_ingredient(recipe,"medium shrimp");
	for (int i=0; i<10; i++)
		if (strlen(recipe[i])>0) printf("%s\n",recipe[i]);
 
	printf("\n ****** substituting ingredient 'soymilk' in recipe\n");
