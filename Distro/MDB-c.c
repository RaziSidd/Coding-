 ...
 *
 * Student Name (Last name, First name): Siddiqui Razi
 * Student Number: 1006419038	
 * UTORid: siddi899
 ...
 *
 * Sign here with your name: Razi Siddiqui
 *
 *
 */
...
ReviewNode *newMovieReviewNode(char *title, char *studio, int year, double BO_total, int score)
{
	...
	ReviewNode *newReviewNode=NULL;
	
	newReviewNode =(ReviewNode *)calloc(1, sizeof(ReviewNode));
	if (score < 0 || score > 100 || year < 1920 || year > 2999 ||strlen(title) > 1024 || strlen(studio) > 1024 ){
		return NULL;
	}
	newReviewNode -> review.score = score; 
	newReviewNode -> review.year = year;
	newReviewNode -> review.BO_total = BO_total; 
	strcpy(newReviewNode->review.movie_title,title);
	strcpy(newReviewNode->review.movie_studio,studio);
	newReviewNode -> review.cast = NULL; 

	newReviewNode -> next = NULL;
	

    return newReviewNode;  
}
...
ReviewNode *findMovieReview(char *title, char *studio, int year, ReviewNode *head)
{
    ...
    /**********  TODO: Complete this function **********************************/
    /***************************************************************************/
	ReviewNode *pointer = NULL;
	pointer = head;
	bool found = false;
	while(pointer != NULL && found == false){
		if (0 == strcmp(pointer -> review.movie_studio, studio) && 0 == strcmp(pointer -> review.movie_title, title) && pointer -> review.year == year){
			found = true;	
		}
		if (found == true){
			return pointer;
                              ...
                              ReviewNode *head)
{
	...
	if (findMovieReview(title, studio, year, head) == NULL){
		if (newMovieReviewNode(title, studio, year, BO_total, score) != NULL){
                       ...
                       ReviewNode *head)
{
	...
	if (findMovieReview(title, studio, year, head) == NULL){
		...
		pointer = findMovieReview(title, studio, year, head);
		if (score < 0 || score > 100 || year < 1920 || year > 2999 || strlen(title) > 1024 || strlen(studio) > 1024 ){
			printf("Sorry, no such movie exists at this point");
		}else{
...
ReviewNode *deleteMovieReview(char *title, char *studio, int year, ReviewNode *head)
{
	...
	follower = head;
	if (0 == strcmp(follower -> review.movie_studio, studio) && 0 == strcmp(follower -> review.movie_title, title) && follower -> review.year == year){     
		free(follower);     
		return pointer;
	}
...
double queryReviewsByStudio(char *studio, ReviewNode *head)
{
	...
	double total = 0.0;
	while (pointer != NULL){
		if(strcmp(pointer->review.movie_studio, studio) ==0){
			printf("%s\n",pointer->review.movie_title);
			printf("%s\n",pointer->review.movie_studio);
			printf("%d\n",pointer->review.year);
			printf("%f\n",pointer->review.BO_total);
...
double queryReviewsByScore(int min_score, ReviewNode *head)
{
	...
	double total = 0.0;
	while (pointer != NULL){
		if(pointer->review.score >= min_score){
			printf("%s\n",pointer->review.movie_title);
			printf("%s\n",pointer->review.movie_studio);
			printf("%d\n",pointer->review.year);
			printf("%f\n",pointer->review.BO_total);
...
ReviewNode *deleteReviewList(ReviewNode *head)
{
    ...
    ReviewNode *pointer=NULL;
	ReviewNode *qointer=NULL;
	pointer=head;
	while (pointer!=NULL){
		qointer=pointer -> next;     
		free(pointer);    
		pointer = qointer;
	}
...
ReviewNode *sortReviewsByTitle(ReviewNode *head)
{
	...
	ReviewNode *pointer=NULL, *temp = NULL;
    MovieReview temprev;
    pointer = head;
    temp = pointer;
    ...
    while(pointer != NULL){
        ...
        while (temp->next !=NULL){
           ...
           if(strcmp(temp->review.movie_title, temp->next->review.movie_title) < 0){ // change the < to a > if needed
				//printf("%d \n", strcmp(temp->review.movie_title, temp->next->review.movie_title));
				temprev = temp->review;
				temp->review = temp->next->review;
				temp->next->review = temprev;
            }
         temp = temp->next;    
        }
        pointer = pointer->next;    
...
CastList *newCastNode(char *name)
{
    /***************************************************************************/
    /**********  TODO: Complete this function *********************************/
    /***************************************************************************/
	
	CastList *newCastNode=NULL;	
	newCastNode =(CastList *)calloc(1, sizeof(CastList));
	strcpy(newCastNode->name,name);
	
	newCastNode -> next = NULL;
    return newCastNode;  
}
void insertCastMember(char *title, char *studio, int year, ReviewNode *head, char *name)
{
	...
	ReviewNode *pointer = NULL;
	if (findMovieReview(title, studio, year, head) != NULL){
		// get the  pointer to the first thing
		pointer = findMovieReview(title, studio, year, head);
		// make a new node for Castlist
		CastList *newCast = NULL;
		// add the name to it
		newCast = newCastNode(name);
		// make the next thing the head of the casts
		newCast -> next = pointer -> review.cast;
		// make the pointer at cast point to newCast
		pointer -> review.cast = newCast; 
...
int countNames(MovieReview *movie, char *name)
{
	...
	int total_names = 0;
	while (pointer != NULL){
		if (NULL != strstr(pointer -> name, name)){
			total_names++;
		}
    ...
    return total_names;  // Remove this when you start working on your solution
}
